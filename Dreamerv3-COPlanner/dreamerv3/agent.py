...
class Agent(nj.Module):
  ...
  def __init__(self, obs_space, act_space, step, config):
    ...
    self.step = step
    if config.is_mpc == 1:
      self.expl_rewfn = expl.Disag(config, name='disag_reward')
      self.wm = WorldModel(obs_space, act_space, config=config, expl=self.expl_rewfn, name='wm')
    else:
      self.wm = WorldModel(obs_space, act_space, config=config, expl=None, name='wm')
    self.task_behavior = getattr(behaviors, config.task_behavior)(
        self.wm, self.act_space, self.config, name='task_behavior')
    # if config.expl_behavior == 'None':
    #   self.expl_behavior = self.task_behavior
    if config.is_lexa == 1:
      self.config = self.config.update({"expl_behavior": 'Explore'})
      print(self.config.expl_behavior)
  ...
  def policy(self, obs, state, mode='train'):
    ...
    if self.config.is_mpc == 1:
      task_output, coplanner_actions = self.task_behavior.policy(latent, task_state)
      task_outs, task_state = task_output
    else:
    ...
    if mode == 'eval':
      ...
      if self.config.is_mpc==1:
        outs['log_entropy'] = outs['action'].entropy()
        if self.config.is_explore==1:
          outs['action'] = coplanner_actions
        else:
          outs['action'] = outs['action'].sample(seed=nj.rng())
      else:
        outs['log_entropy'] = outs['action'].entropy()
  ...
  def train(self, data, state):
    ...
    if self.config.is_mpc==1:
      expl_rewfn_mets = self.expl_rewfn.train(context)
      metrics.update(expl_rewfn_mets)
    _, mets = self.task_behavior.train(self.wm.imagine, start, context)
    metrics.update(mets)
    if self.config.expl_behavior != 'None' or self.config.is_lexa == 1:
...
class WorldModel(nj.Module):
  ...
  def __init__(self, obs_space, act_space, config, expl):
    self.obs_space = obs_space
    self.act_space = act_space['action']
    self.config = config
  ...
  def imagine(self, policy, start, horizon):
    ...
    def step(prev, _):
      prev = prev.copy() #[deter, logit, stoch, action]
      state = self.rssm.img_step(prev, prev.pop('action'))
      if self.config.is_mpc == 1 and self.config.is_rollout == 1:
        ...
        for h in range(self.config.coplanner_horizon):
            ...
            mpc_next_state = self.rssm.img_step(mpc_state, mpc_actions)
            mpc_next_actions = policy(mpc_next_state)
        ...
        rewfn = lambda s: self.heads['reward'](s).mean()
        state_copy["action"] = actions
        print(state_copy)
        rew = rewfn(state_copy)
        ...
        total_reward = total_reward - self.config.conservative_rate * model_uncert

        index = jnp.argmax(total_reward, axis=0)
        final_actions = action_candidates[index, jnp.arange(index.shape[0]), :]
        return {**state, 'action': final_actions}
      else:
