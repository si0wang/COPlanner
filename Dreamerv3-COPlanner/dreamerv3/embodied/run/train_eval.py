...
def train_eval(
    agent, train_env, eval_env, train_replay, eval_replay, logger, args, config):
  ...
  exp_dir = config.logdir
  if not os.path.isdir(exp_dir):
    os.makedirs(exp_dir)
  if config.is_mpc==1:
    log_file = os.path.join(exp_dir, 'coplanner_{}_{}_{}_{}.txt'.format(config.task, config.seed, config.conservative_rate, config.optimistic_rate))
  else:
    log_file = os.path.join(exp_dir, '{}_{}.txt'.format(config.task, config.seed))
  logger_log = logging.getLogger(__name__)
  logger_log.setLevel(level=logging.INFO)
  handler = logging.FileHandler(log_file)
  handler.setLevel(logging.INFO)
  formatter = logging.Formatter('%(asctime)s - %(message)s')
  handler.setFormatter(formatter)
  logger_log.addHandler(handler)
  logger_log.info('n_trajs: {}, horizon: {}, conservative: {}, optimistic: {}'.format(config.n_trajs, config.coplanner_horizon, config.conservative_rate, config.optimistic_rate))
  ...
  def per_episode(ep, mode):
    ...
    print(f'{mode}_Episode has {length} steps and return {score:.1f}.')
    if mode == 'eval':
      logger_log.info('Steps: {}, Score: {}'.format(step, score))
    stats = {}
  ...
  checkpoint = embodied.Checkpoint(logdir / config.exp_name / config.seed / 'checkpoint.ckpt')
  ...
  while step < args.steps:
    if should_eval(step):
      print('Starting evaluation at step', int(step), logger.step, len(train_replay))
      driver_eval.reset()
