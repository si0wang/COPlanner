...
class PredictEnv:
    ...
    def step(self, obs, act, deterministic=False):
        ...
        # samples_for_uncertainty = np.reshape(ensemble_samples, [args.num_elites, args.ntraj, args.rollout_batch_size, 1])

        uncertainty = np.mean(np.std(ensemble_samples, axis=0), axis=-1)
        ...
        terminals = self._termination_fn(self.env_name, np.reshape(obs, [-1, obs.shape[-1]]), np.reshape(act, [-1, act.shape[-1]]), next_obs)
        ...
        return next_obs, rewards, uncertainty, terminals, info
