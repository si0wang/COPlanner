...
class ImagBehavior(nn.Module):
  ...
  def _train(
      self, start, objective=None, expl_behavior=None, is_training_policy=1, action=None, reward=None, imagine=None, tape=None, repeats=None, is_intrinsic=None):
    ...
    with tools.RequiresGrad(self.actor):
      with torch.cuda.amp.autocast(self._use_amp):
        ...
        imag_feat, imag_state, imag_action = self._imagine(
          start, self.actor, self._config.imag_horizon, expl_behavior, is_training_policy, repeats)
        end_time = time.time()

        # print("image time: {}".format(end_time-start_time))
    ...
    end_time = time.time()
    # print("mid time: {}".format(end_time-start_time))

    with tools.RequiresGrad(self):
      start_time = time.time()
      metrics.update(self._actor_opt(actor_loss, self.actor.parameters()))
      end_time = time.time()
      print("policy learning time: {}".format(end_time - start_time))
      metrics.update(self._value_opt(value_loss, self.value.parameters()))

    return imag_feat, imag_state, imag_action, weights, metrics

  def _imagine(self, start, policy, horizon, expl_behavior, is_training_policy, repeats=None):
    ...
    def step(prev, _):
      ...
      ## Fixme: MPC rollout for action selection
      if self._config.is_mpc == 1:
        ...
        feat = dynamics.get_feat(state_copy)

        action = policy(feat.clone()).sample()
        action_candidates = action.clone()

        reward_func = lambda f, s, a: self._world_model.heads['reward'](
          self._world_model.dynamics.get_feat(s)).mode()
        expl_func = expl_behavior
        ...
        for h in range(self._config.planning_horizon):
            ...
            next_state = dynamics.img_step(state_4_img, action_4_img, sample=self._config.imag_sample)
            next_feat = dynamics.get_feat(next_state)
            next_action = policy(next_feat).sample()
        ...
        reward = reward_func(dynamics.get_feat(state_copy), state_copy, action)
        ...
        model_uncertainty = expl_func._intrinsic_reward(dynamics.get_feat(state_copy), state_copy, action)
        ...
        total = reward - self._config.conservative_rate * model_uncertainty
      ...
      ## Fixme: original action selection
        feat = dynamics.get_feat(state)
        inp = feat.detach() if self._stop_grad_actor else feat
        action = policy(inp).sample()

      succ = dynamics.img_step(state, action, sample=self._config.imag_sample)
      return succ, dynamics.get_feat(state), action

    feat = 0 * dynamics.get_feat(start)
    action = policy(feat).mode()
    succ, feats, actions = tools.mpc_static_scan(
