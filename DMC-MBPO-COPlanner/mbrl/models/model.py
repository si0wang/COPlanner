...
class Model(nn.Module, abc.ABC):
    ...
    def load(self, load_dir: Union[str, pathlib.Path]):
        ...
        self.load_state_dict(torch.load(pathlib.Path(load_dir) / self._MODEL_FNAME, map_location=torch.device('cpu')))
...
class Ensemble(Model, abc.ABC):
    ...
    def sample_1d_w_ovr(
        ...
        sample_mean, sample_logvar = self.forward(
            model_input,
            rng=rng,
            propagation_indices=model_state["propagation_indices"],
        )
        ensemble_vars = ensemble_logvars.exp()
        sample_var = sample_logvar.exp()
        sample_std = torch.sqrt(sample_var)
        E = ensemble_means.shape[0]
        rest_mean = (torch.sum(ensemble_means, dim=0) - sample_mean) / (E - 1)
        rest_var = (torch.sum(ensemble_vars + ensemble_means ** 2, dim=0) - (sample_var + sample_mean ** 2)) / (
                    E - 1) - rest_mean ** 2
        rest_std = torch.sqrt(rest_var)

        sample_dist = Normal(sample_mean, sample_std)
        rest_dist = Normal(rest_mean, rest_std)

        ovr_score = torch.sum(kl_divergence(sample_dist, rest_dist), dim=-1)
        if deterministic or self.deterministic:
            return sample_mean, ovr_score, model_state
        return sample_dist.sample(), ovr_score, model_state

    def sample_1d_ensemble(
            self,
            model_input: torch.Tensor,
            model_state: Dict[str, torch.Tensor],
            deterministic: bool = False,
        ...
        ensemble_vars = ensemble_logvars.exp()
        ensemble_stds = torch.sqrt(ensemble_vars)

        ensemble_samples = torch.normal(ensemble_means, ensemble_stds)

        num_models, batch_size, _ = ensemble_samples.shape
        model_idxes = np.random.choice(np.arange(0, num_models), size=batch_size)
        ...
        # print(samples.shape)
        uncertainty = torch.mean(torch.std(ensemble_samples, dim=0), dim=-1)

        # print(uncertainty.shape)
        #
        # print(torch.normal(means, stds, generator=rng).shape)

        return samples, uncertainty, model_state